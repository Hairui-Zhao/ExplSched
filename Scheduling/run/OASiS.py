...
def assign_resource(job):
    ...
    if (job.name=="de" and job.net=="169") or (job.name=="re" and job.net=="152") or(job.name=="de" and job.net=="201") or(job.name=="in"):
        if GPU_state.count(0)>=4:
            ...
            return 0
    elif (job.name=="vg" and job.net=="13") or (job.name=="vg" and job.net=="19") or  (job.name=="re" and job.net=="50") or (job.name=="vg" and job.net=="11"):
...
with open("/root/ExplSched/Scheduling/run/JCT.txt","w+") as Jct:
    for finish_job in finish_queue:
        ...
        msg=finish_job.ID+" "+str(f_t)+" "+str(finish_job.exe_s_time-xxxtime)+" "+str(finish_job.end_time-xxxtime)
        Jct.write(msg)
        Jct.write("\n")